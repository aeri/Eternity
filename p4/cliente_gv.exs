...
defmodule ClienteGV do
  ...
  def obten_vista(nodo_cliente) do
    ...
    # esperar respuesta del latido
    receive do
      {:vista_vista_v, vista, is_ok?} -> {vista, is_ok?}
    after
      @tiempo_espera_de_respuesta ->
        {ServidorGV.vista_inicial(), false}
    end
  ...
  defp procesa_obten_vista(nodo_servidor_gv, pid_maestro) do
    ...
    receive do
      {:vista_vista_v, vista, coincide?} ->
        send(pid_maestro, {:vista_vista_v, vista, coincide?})
    after
      @tiempo_espera_de_respuesta ->
        send(pid_maestro, {:vista_vista_v, ServidorGV.vista_inicial(), false})
    end
  end
end
